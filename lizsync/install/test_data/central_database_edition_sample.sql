-- Modify some data in central database
INSERT INTO "test"."montpellier_districts" (fid,uid,url,geom,libquart,quartmno)
VALUES (
    null,'b2ad98a8-1627-7eb4-fc6f-5a7ce2583bd5',null,'','Nouveau quartier','NEWQUART'
);

UPDATE "test"."montpellier_sub_districts" SET
    libsquart = concat(libsquart, ' by centrale'),
    squartmno = concat(squartmno, ' by centrale')
WHERE ogc_fid = 22;

DELETE FROM "test"."montpellier_sub_districts" WHERE libsquart = 'Gares';

UPDATE "test"."pluviometers" SET
    nom = concat(nom, ' by central'),
    geom = ST_Translate(geom, 2, 4)
WHERE nom = 'pluvio18';

UPDATE "test"."pluviometers" SET
    geom = '01010000206A0800009E6D82E3057527418D1435F092F65741'
WHERE nom = 'pluvio3';

INSERT INTO "test"."pluviometers" (id, nom, geom)
VALUES (
    98, 'pluvio98', ST_SetSRID(ST_MakePoint(771164, 6281875),2154)
);
