...
echo "# IMPORT DATA"
for f in install/test_data/data/*.geojson; do ogr2ogr -overwrite -f PostgreSQL  "PG:service=$SERVICE active_schema=$SCHEMA" $f -lco geometry_name=geom -lco fid=ogc_fid; done;
